function netmapApp() {
  return {
    ...
    q: "",
    deep: false,       // Checkbox de modo profundo
    workers: 12,
    lastDeep: false,   // Si el último escaneo fue deep
    refreshSeconds: 10,
    timer: null,
    job: null,
    status: null,
    ...
    async loadDevices() {
      try {
        ...
        // Detectar si el escaneo fue deep (si hay TTL o puertos)
        this.lastDeep = newList.some(d => (d.ttl !== null && d.ttl !== undefined) || (d.open_ports && d.open_ports.length));
    ...
    async scan() {
      ...
      try {
        const url = this.deep 
          ? `/api/scan?deep=true&workers=${this.workers}` 
          : `/api/scan?deep=false`;
    ...
    reloadGraph() {
      ...
      if (iframe) iframe.src = '/api/graph?ts=' + Date.now();
    },
